...
def _safe_json(data: bytes) -> object:
    """Try to parse as JSON; return the parsed object or None."""
    try:
...
def _dump_flow(flow: http.HTTPFlow, index: int, cd: Path):
    ...
    # Write binary dumps for large / non-UTF-8 bodies.
    if len(req_body) > 256 * 1024 or _safe_json(req_body) is None and req_body:
        bin_path = cd / f"flow.{index}.req.bin"
        bin_path.write_bytes(req_body)
    if response and (len(resp_body) > 256 * 1024 or _safe_json(resp_body) is None and resp_body):
        bin_path = cd / f"flow.{index}.resp.bin"
        bin_path.write_bytes(resp_body)
...
class Capture:
    ...
    def error(self, flow: http.HTTPFlow):
        # capture flows that fail before a response arrives.
        self._do_dump(flow)
