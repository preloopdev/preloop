...
cargo test --quiet template_string_token_handles_braces_inside_string_literals --lib --manifest-path crates/aksh-gha-protocol/Cargo.toml

# Compile a test for literal brace case
echo 'fn main() {
    use aksh_gha_protocol::azdo::job::template_string_token;
...
RUST_BACKTRACE=1 cargo run --quiet --manifest-path crates/aksh-gha-protocol/Cargo.toml --example dummy --manifest-path <(echo '[package] name="dummy" version="0.1" edition="2021" [dependencies] aksh-gha-protocol = { path = "crates/aksh-gha-protocol" } serde_json = "1.0"') -- /tmp/repro_mc_s6.rs 2>&1 | cat
