...
#[derive(Debug, Parser)]
#[command(name = "preloop-conformance")]
#[command(about = "Run preloop conformance checks against upstream runner.server fixtures")]
struct Cli {
    #[command(subcommand)]
    command: CommandKind,
}
...
enum CommandKind {
    ...
    /// Run an upstream command and an preloop command, then compare stdout exactly.
    CompareCommand {
        /// Upstream command executable.
        #[arg(long)]
        upstream: String,
        /// preloop command executable.
        #[arg(long)]
        preloop: String,
...
async fn main() -> anyhow::Result<()> {
    match Cli::parse().command {
        ...
        CommandKind::CompareCommand {
            ...
            preloop,
            args,
        } => compare_command(upstream, preloop, args).await,
        ...
        CommandKind::RunnerE2e {
            ...
            record_flows,
            port,
        } => run_runner_e2e(runner_bin, workflow, record_flows, port).await,
...
async fn compare_command(
    ...
    let preloop_output = Command::new(&preloop)
        .args(&args)
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .output()
        .await
        .with_context(|| format!("run preloop command `{preloop}`"))?;

    if upstream_output.stdout != preloop_output.stdout {
        bail!(
            "stdout mismatch\n--- upstream ---\n{}\n--- preloop ---\n{}",
            String::from_utf8_lossy(&upstream_output.stdout),
            String::from_utf8_lossy(&preloop_output.stdout)
        );
    }
    if upstream_output.status.success() != preloop_output.status.success() {
        bail!(
            "exit status mismatch: upstream={} preloop={}",
            upstream_output.status,
            preloop_output.status
        );
    }
    Ok(())
}
...
async fn replay_wire(fixtures: PathBuf) -> anyhow::Result<()> {
    ...
    eprintln!("  1. Record traffic with 'preloop-conformance record'");
    eprintln!(
        "  2. Run 'preloop-conformance replay --fixtures {}'",
        fixtures.display()
    );
...
fn preseed_private_actions(
    ...
    let search_str = "preloopdev/preloop/.github/actions/tier2-property-oracle@";
    let mut cursor = 0;
...
async fn run_runner_e2e(
    ...
    // Start server in background on the requested port
