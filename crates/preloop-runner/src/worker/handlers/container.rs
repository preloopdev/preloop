...
fn container_action_env(
    ...
    _expr_ctx: Option<&preloop_gha_expressions::Context>,
) -> Result<std::collections::HashMap<String, String>> {
...
fn lifecycle_entry(with: &serde_json::Value) -> Option<&str> {
    with.get("__preloop_entry").and_then(|v| v.as_str())
}
...
mod tests {
    ...
    #[test]
    fn evaluated_inputs_skips_preloop_internal_keys() {
        let mut job = crate::worker::contexts::JobContext::new(
            "job".into(),
            "job".into(),
            serde_json::json!({}),
        ...
        let inputs = evaluated_inputs(
            None,
            &serde_json::json!({"__preloop_entry": "pre.js", "real": "val"}),
            &ctx,
        )
        .unwrap();
