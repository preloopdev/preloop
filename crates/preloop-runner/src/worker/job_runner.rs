...
pub async fn run_job(
    ...
    // Captured before lifecycle expansion: `main_steps` is 1:1 with the job
    // request message's `steps`, so this is the workflow's own declaration.
    // Everything `build_step_list_with_lifecycle` and the hook injections add
    // below is runner-generated by construction.
    ...
    }
    {
        ...
        if enable_debugger {
            ...
            let debugger_welcome = job_message
                ...
                .and_then(|v| v.as_str())
                .map(str::to_string);
            let debugger_transport = preloop_dap::DebuggerTransportMode::from_wire(
                job_message
                    .get("preloopDebugTransport")
                    .and_then(|v| v.as_str()),
            );
    ...
    if let Some(dbg) = job_ctx.dap_debugger.as_ref() {
        let entries: Vec<preloop_dap::SourceEntry> = ordered_steps
            .iter()
            .map(|s| {
            ...
            .collect();
        let post = vec![preloop_dap::SourceEntry {
            display_name: "Complete job".into(),
            is_pre: false,
            is_post: true,
        }];
        let predicted = vec![preloop_dap::PredictedPostStep {
            display_name: "Complete job".into(),
            frame_id: 1,
        }];
...
async fn build_debug_pause_client(
    ...
    let run_id = job_message
        .get("preloopDebugRunId")
        .and_then(|v| v.as_str())
        .ok_or_else(|| anyhow::anyhow!("job message carries no preloopDebugRunId"))?
        .parse::<preloop_gha_protocol::RunId>()
        .map_err(|error| anyhow::anyhow!("preloopDebugRunId is not a run id: {error}"))?;
    ...
    Ok(super::debug_pause::DebugPauseClient::acquire(
        ...
        &runtime_token,
        run_id,
        preloop_gha_protocol::JobId(job_id.to_owned()),
        agent_job_id,
        job_name.to_owned(),
    )
    .await?
