...
/// Whether a resolved step was generated by the runner rather than declared
/// in the workflow. The index remains in the resolved list for retry fidelity,
/// while the debug CLI hides these steps from workflow-facing counts.
fn is_internal_step(step: &Step) -> bool {
    step.id.starts_with("__hook_")
        || step
            .raw
            .get("__pre")
        ...
        if debug_client.is_some() {
            steps
                ...
                .map(|(i, s)| preloop_gha_protocol::debug_session::StepSummary {
                    ...
                    is_internal: is_internal_step(s),
                })
                .collect()
        } else {
            Vec::new()
