...
/// Whether a resolved step was generated by the runner rather than declared in
/// the workflow.
///
/// Membership in the job request message's step list is the whole test. The
/// message is the authoritative declaration of what the workflow asked for:
/// every declared step is in it, and every step the runner synthesizes
/// (`Set up job`, `Pre`/`Post` action hooks, container lifecycle, host hooks,
/// `Complete job`) is not. Verified against the official runner in
/// `.runner-watch/golden/v2.336.0/06-multi-step`, where the three declared
/// steps echo the message ids back and the two runner-generated ones carry ids
/// absent from it.
///
/// Deliberately not a prefix or marker heuristic. `job_builder` passes a user's
/// `id:` through verbatim as the context name, so a workflow that declares
/// `id: __pre_build` would have its own step classified as lifecycle; and
/// lifecycle steps parsed from a live payload can carry a GUID id with the
/// markers spread across `raw.__pre`, `raw.__post`, `raw.isPre` and
/// `raw.isPost`, so any prefix list is a shape away from being wrong.
///
/// The index stays in the resolved list for retry fidelity; the debug CLI hides
/// these steps from workflow-facing counts.
fn is_synthetic_step(step: &Step, declared_step_ids: &std::collections::HashSet<String>) -> bool {
        ...
        if debug_client.is_some() {
            steps
                ...
                .map(|(i, s)| preloop_gha_protocol::debug_session::StepSummary {
                    ...
                    synthetic: is_synthetic_step(s, &job.declared_step_ids),
                })
                .collect()
        } else {
            Vec::new()
