                    ...
                    Ok(Some(msg)) => {
                        ...
                        match classify_message(&message_type) {
                            ...
                            BrokerMessageKind::AgentRefresh => {
                                info!("Self-update requested; preloop-runner does not self-update");
                            }
                            ...
                            BrokerMessageKind::RunnerRefresh => {
                                info!("Self-update requested (RunnerRefresh); preloop-runner does not self-update");
                            }
...
fn extract_session_key_if_present(
    ...
    if encrypted {
        ...
        let hash = if use_fips_encryption {
            ...
            preloop_gha_protocol::crypto::RsaOaepHash::Sha1
        };
...
fn announce_busy() {
    ...
    let _ = writeln!(stdout, "{}", preloop_gha_protocol::RUNNER_BUSY_SENTINEL);
    let _ = stdout.flush();
}
