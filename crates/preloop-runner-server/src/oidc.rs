...
fn generate_certificate(keypair: &AgentRsaKeypair) -> Result<Vec<u8>, anyhow::Error> {
    ...
    params
        .distinguished_name
        .push(DnType::CommonName, "preloop OIDC");
...
mod lie_github_tests {
    ...
    fn our_claims_match_github_golden_key_set() {
        let input = OidcClaimsInput {
            repository: "preloopdev/preloop-conformance-sample".to_string(),
            ...
            repository_owner_id: "297229730".to_string(),
            workflow_ref: Some(
                "preloopdev/preloop-conformance-sample/.github/workflows/15-oidc-id-token.yml@refs/heads/main"
                    .to_string(),
            ),
        ...
        assert_eq!(
            claims["sub"],
            "repo:preloopdev/preloop-conformance-sample:ref:refs/heads/main"
        );
        assert_eq!(claims["aud"], "api://preloop");
