...
async fn resolve_ref_to_sha(
    ...
    if let Ok(cache) = state.action_sha_cache.lock() {
        if let Some((sha, at)) = cache.get(&cache_key) {
            if at.elapsed() < ACTION_SHA_CACHE_TTL {
