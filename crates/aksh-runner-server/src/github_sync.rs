...
pub(crate) async fn sync_run_to_github(
    ...
    let token = match sync_token(shared, &repository).await {
        ...
        None => {
            let message =
                "no GitHub credentials configured (GitHub App or AKSH_GITHUB_TOKEN)".to_owned();
    ...
    let base = match &{
        ...
        inner
            .runs
            .get(&run_id)
            .and_then(|r| r.submission.base_ref.clone())
    } {
        Some(base) => base
            .strip_prefix("refs/heads/")
            .map(str::to_owned)
    ...
    let pr_number = if let Some(number) = pr_number {
        ...
        .await
        {
            Ok(pr) => pr.get("number").and_then(|n| n.as_u64()),
            Err(error) => {
    ...
    for job_id in jobs.keys() {
        let has_check_run = {
            ...
            inner
                .runs
                .get(&run_id)
                .is_some_and(|r| r.job_check_run_ids.contains_key(job_id))
        };
        if !has_check_run {
...
const fn default_zero_sha() -> &'static str {
