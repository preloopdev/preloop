    ...
    for (job_id, job) in &workflow.jobs {
        for matrix in expand_matrix(job_id, job.strategy.matrix.as_ref())? {
            ...
            plans.push(JobPlan {
                ...
                job_outputs: job.outputs.iter().map(|(k, v)| {
...
fn coerce_value(val: &Value, input_type: InputType, input_name: &str) -> Result<Value, ParserError> {
    match input_type {
...
"#
    ...
    mod coerce_value_properties {
        ...
        fn accepts_true_false_strings_for_boolean() {
            assert_eq!(coerce_value(&Value::String("true".into()), InputType::Boolean, "f").unwrap(), Value::Bool(true));
            assert_eq!(coerce_value(&Value::String("false".into()), InputType::Boolean, "f").unwrap(), Value::Bool(false));
            assert_eq!(coerce_value(&Value::String("TRUE".into()), InputType::Boolean, "f").unwrap(), Value::Bool(true));
        }

        #[test]
        fn accepts_numeric_strings_for_number() {
            assert_eq!(coerce_value(&Value::String("42".into()), InputType::Number, "n").unwrap(), Value::Number(42.into()));
            assert_eq!(coerce_value(&Value::String("-7".into()), InputType::Number, "n").unwrap(), Value::Number((-7).into()));
        }
