...
//!
//! Wires `preloop-gha-expressions` to resolve `${{ }}` in workflow fields
...
pub fn validate_expressions_in_string(
    ...
    if is_condition && !input.contains("${{") {
        let effective = preloop_gha_expressions::effective_condition(Some(input));
        preloop_gha_expressions::validate_expression(&effective)
    ...
    while let Some(start) = remaining.find("${{") {
        ...
        preloop_gha_expressions::validate_expression(expr)
