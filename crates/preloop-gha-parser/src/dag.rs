...
pub fn validate_job_plans(jobs: &[JobPlan]) -> Result<(), ParserError> {
    ...
    for job in jobs {
        if let Some(condition) = &job.if_condition {
            let effective = preloop_gha_expressions::effective_condition(Some(condition));
            preloop_gha_expressions::validate_expression(&effective).map_err(|error| {
...
mod tests {
    ...
    fn job_plan(id: &str, base: &str, needs: &[&str]) -> JobPlan {
        JobPlan {
            id: preloop_gha_protocol::JobId(id.to_owned()),
            ...
            needs: needs
                .iter()
                .map(|n| preloop_gha_protocol::JobId(n.to_string()))
                .collect(),
