...
//!
//! This lived only in the `preloop-conformance` `golden` subcommand, which is a
...
//!
//! ```sh
//! UPDATE_GOLDEN=1 cargo test -p preloop-gha-parser --test golden
//! ```
//!
...
fn golden_expansions_match_fixtures() {
    ...
    assert!(
        ...
        "golden expansion drift in {mismatched:?}; \
         ...
         `UPDATE_GOLDEN=1 cargo test -p preloop-gha-parser --test golden` and review the diff"
    );
}
