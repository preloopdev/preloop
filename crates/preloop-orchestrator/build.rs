...
fn main() {
    ...
    // Pin sources: versions.toml plus official-image.toml (official runner
    // bases live there so their bumps don't trigger on unrelated versions
    // churn). Same flat format; keys must be unique across both files.
    let pin_files = [
        manifest_dir.join("../../versions.toml"),
        manifest_dir.join("../../official-image.toml"),
    ];
    let mut pins: BTreeMap<String, String> = BTreeMap::new();
    let mut sha_pins: BTreeMap<String, String> = BTreeMap::new();
    // Assigned at the top of every file iteration below.
    let mut in_sha_section: bool;

    for versions_path in &pin_files {
        let text = fs::read_to_string(versions_path).expect("pin file must exist");
        // Sections never span files; reset per file so a trailing section
        // in one file cannot misclassify the next file's keys.
    ...
    let mut out = String::from(
        "// Generated by build.rs from versions.toml and official-image.toml. Do not edit.\n",
    );
    ...
    println!("cargo:rerun-if-changed=../../official-image.toml");
}
