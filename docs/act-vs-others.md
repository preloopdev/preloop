...
- **GitHub-hosted runners** — the reference behavior and zero maintenance,
  ...
  baseline that aksh also speaks: aksh replaces the GitHub side of the wire
  protocol, not the runner side.

The rest of this document is the three-way deep dive (act vs agent-ci vs
aksh); the table rows marked agent-ci apply to the Gitea/Forgejo family too,
since `act_runner` shares act's engine.
