...
## 0. Current Container-Support Recommendation

For GitHub Actions `container:` / `services:` compatibility, use **Docker engine behavior first**.
The official runner uses Docker for job containers and service containers, so the shortest correct path is:
...
Hosted/prod mode:
  ...
  └── fresh job VM/microVM
      ...
      ├── dockerd
      ├── containerd
      ├── runc or Docker's configured OCI runtime
      └── job/service containers
```

`libkrun` does **not** bundle `dockerd`, `containerd`, or `runc`. A libkrun-based Docker runner means building a Docker-capable Linux guest image and booting that image per job. The Docker-less `crun`/libkrun path remains useful research, but it is not the initial compatibility plan: `crun` is an OCI runtime, not a Docker engine, and would require aksh to implement Docker behavior itself.

Use this document's Firecracker/libkrun/security notes to choose the VM substrate. Use `docs/runner/container-job-services-plan.md` as the current implementation plan for container support.
