...
| Quick diff harness | `scripts/diff.sh "SQL"` | 🚧 `just conform-local`/`conform-runner`/`conform-smoke` exist but are heavier (full conform run, not one query) | Add `just diff-trace <workflow>` one-liner over the existing `compare::render_report` |
