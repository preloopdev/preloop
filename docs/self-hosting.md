...
`preloop serve` is the whole server: Unfortunately, for now, the control plane (runner protocol, native API, GitHub Checks reporting) plus the microVM runner pool that executes jobs are in one binary. Next release will separate the two, 
...
if you can live without webhook-triggered runs, you can run with **no inbound access at all**. see option A.
...
local development, WSL/Windows is untested as a server.
...
Then install it as a supervised service using systemd on Linux, launchd on macOS: More deployment methods coming soon. 
...
| `PRELOOP_STORE_URL`        | SQLite in the state dir     | `sqlite://<path>`, a bare path, or `postgres://…?sslmode=require              |
...
Webhook URL: `https://[[ORCA_RICH_MD:9c5f3664c1e6c271b7ece30bae9e3266:inline-html:%3Chost%3E]].[[ORCA_RICH_MD:9c5f3664c1e6c271b7ece30bae9e3266:inline-html:%3Ctailnet%3E]].ts.net/api/v1/github/webhooks`
...
### D. Your own domain without a third party
...
3. Keep preloop on loopback and expose only the webhook path through something like caddy.
...
nstallation token and any secrets scoped to that job, and
