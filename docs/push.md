...
| **Remote server** (`AKSH_URL`)     | Same commands; the server runs against its own configured workspace (the local-workspace header is loopback-only so the server never reads your disk), and the push still happens from your checkout |
