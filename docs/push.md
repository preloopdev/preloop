...
Normally a PR's CI depends on GitHub delivering a webhook: push event →
server creates the run. If GitHub's webhook pipeline is down, or your
tunnel/secret is broken, **no runs get created at all** — CI goes dark even
for merged PRs, and GitHub stops retrying missed deliveries after ~24h.

The submit-driven flow inverts the dependency. **You submit the CI run to
the server directly; the server runs it; then — if you ask — it pushes the
tested commit to GitHub and opens or updates the pull request.** GitHub is
the *sink* for results, not the required *source* of triggers. PRs get
...
|---|---|
| `preloop run -f <workflow>` | Run CI on the server. **Zero GitHub involvement** — works during GitHub outages |
...
| `preloop push` | Replay push-back for the **most recent** run (idempotent — safe to re-run) |
...
- **The server must have GitHub App credentials.** The Checks API rejects
  ...
  (`preloop setup github --via app`, or `AKSH_GITHUB_APP_ID` + PEM) checks
  pre-fill correctly; a PAT-only server still creates the PR but shows no
  checks.
- The check's details link uses `AKSH_PUBLIC_URL` (set it to the address
...
| **Remote server** (`AKSH_URL`) | Same commands; the server runs against its own configured workspace (the local-workspace header is loopback-only — the server never reads your disk), and the push still happens from your checkout |
