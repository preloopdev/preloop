...
| **Strategy**                                         | Replace the runner binary with a Go reimplementation that executes steps in Docker containers | Reimplement **both** the control plane (server) **and** the runner in Rust, with the full AzDO wire protocol between them     | Replace the server and the runner; speak the official AzDO protocol between them                                                                                | Replace the runner; Gitea supplies scheduling over a proprietary Connect-RPC protocol                   |
