# act vs agent-ci vs preloop: GitHub Actions Local Runners Compared

A detailed comparison between [nektos/act](https://github.com/nektos/act),  
agent-ci, and preloop (this repo) — plus the adjacent options (Gitea/Forgejo  
Actions, GitHub-hosted runners). All of these let you run GitHub Actions  
workflows locally or outside GitHub-hosted machines. They take fundamentally  
different approaches to the problem.

> Numbers below were measured on an Apple M4 Max in July 2026. External facts
> (act's release cadence, stars) are as of that date.
...
|                                                      | act                                                                                           | agent-ci                                                                                                                      | preloop                                                                                                                                                         | Gitea Actions                                                                                           |
| ---------------------------------------------------- | --------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| **One-liner**                                        | Reimplement the runner in Go                                                                  | Reimplement the server in TypeScript                                                                                          | Reimplement server + runner in Rust                                                                                                                             |                                                                                                         |
| **Strategy**                                         | Replace the runner binary with a Go reimplementation that executes steps in Docker containers | Reimplement **both** the control plane (server) **and** the runner in Rust, with the full AzDO wire protocol between them     | Replace the server and the runner; speak the official AzDO protocols between them                                                                               | Replace the runner; Gitea supplies scheduling over a proprietary Connect-RPC protocol                   |
| **Compatibility target**                             | Behavioral: "workflows should mostly work"                                                    | Protocol: "the official runner v2.336.0 cannot tell the difference"                                                           | Protocol: "the official runner v2.336.0 cannot tell the difference"                                                                                             | Workflow-level: GitHub YAML syntax + the action ecosystem on Gitea's own wire protocol                  |
| **Verification method**                              | Manual testing + community bug reports                                                        | Differential conformance vs the official runner                                                                               | Golden wire-capture scenarios replayed against the server; differential conformance vs official runner                                                          | Unit tests + community reports; no official-runner wire conformance (by design)                         |
| **Official runner image / persistent control plane** | ❌ No control plane — single binary that reimplements the runner in Go                         | ✅ Official runner image (unmodified `actions/runner` in Docker); control plane is a per-run local DTU server, no shared state | ❌ No official runner image — own Rust reimplementation (~99% compatible); ✅ persistent control plane (long-lived server: scheduler, broker, concurrency groups) | ❌ act-fork runner; ✅ persistent control plane (the Gitea monolith itself schedules, stores, and serves) |
...
```
┌─────────────────────────────────────────────────────┐
│              preloop-runner-server (Rust, Axum)         │
│                                                     │
...
| Feature                                  | act | agent-ci | preloop | Gitea Actions | Notes                                                  |
| ---------------------------------------- | :---: | :--------: | :-------: | :-------------: | ------------------------------------------------------ |
...
| `workflow_dispatch` inputs               | ✅   | ⚠️       | ✅       | ✅             |                                                        |
| `run-name` expression                    | ❌   | ⚠️       | ✅       | ⚠️            | preloop parses and evaluates at submit time            |
...
| `runner.*` context                | ✅          | ⚠️       | ✅       | ✅             |                                                   |
| `inputs.*` context                | ✅          | ✅        | ✅       | ✅             |                                                   |
| Comprehensive `GITHUB_*` env vars | ⚠️ partial | ⚠️       | ✅       | ⚠️            | preloop injects 40+ vars matching official runner |
...
| `{{` / `}}` escape sequences                           | ❌                      | ❌                            | ✅                   | ✅                               |                                       |
| Case-insensitive `==`                                  | ⚠️                     | ✅                            | ✅                   | ✅                               |                                       |
| Parser                                                 | `rhysd/actionlint` AST | delegated to official runner | Custom Pratt parser | actionlint AST (fork evaluator) | preloop owns its parser               |
...
| Feature                                | act | agent-ci | preloop       | Gitea Actions | Notes                                                                                                                                                                                                                                                                                                                             |
| -------------------------------------- | :---: | :--------: | :-------------: | :-------------: | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Faithful runner reimplementation       | ✅   | ❌        | ✅             | ✅             | act reimplements in Go; preloop reimplements in Rust (faithful port of v2.336.0)                                                                                                                                                                                                                                                  |
| Official runner image / binary         | ❌   | ✅        | ❌             | ❌             | agent-ci runs the official `actions/runner` binary in the official image (same .NET process as GitHub-hosted); act/preloop reimplement (preloop ~99% compatible w 10x less binary sizes and memory RSS); Gitea forks act                                                                                                          |
| AzDO wire protocol                     | ❌   | ⚠️       | ✅             | ❌             | act doesn't speak the protocol                                                                                                                                                                                                                                                                                                    |
| Runner registration handshake          | ❌   | ⚠️       | ✅             | ⚠️            | RSA key exchange, session crypto                                                                                                                                                                                                                                                                                                  |
| Broker acquire/renew/complete          | ❌   | ❌        | ✅             | ⚠️            | Full broker message lifecycle                                                                                                                                                                                                                                                                                                     |
...
| Job permissions / GITHUB_TOKEN scoping | ❌   | ⚠️       | ✅             | ⚠️            | preloop always injects `GITHUB_TOKEN` as a secret (`PRELOOP_GITHUB_TOKEN` PAT or local JWT) and computes effective permissions from the workflow's `permissions:` block. agent-ci (per its own compatibility.md): token injected only with `--github-token`, permissions accepted but not enforced; Gitea: repo-scoped task token |
| Job cancellation (wire protocol)       | ❌   | ❌        | ✅             | ⚠️            | `CancellationTiming` with clamped timeouts                                                                                                                                                                                                                                                                                        |
...
| Feature                            | act                            | agent-ci | preloop    | Gitea Actions | Notes                                                                                                                     |
| ---------------------------------- | :------------------------------: | :--------: | :----------: | :-------------: | ------------------------------------------------------------------------------------------------------------------------- |
| Docker job containers              | ✅                              | ✅        | ✅          | ✅             | Different models: act uses idle container + exec; preloop delegates to the runner                                         |
| Service containers                 | ✅                              | ✅        | ✅          | ✅             |                                                                                                                           |
| Docker-in-Docker                   | ✅                              | ❌        | ✅          | ✅             |                                                                                                                           |
| MicroVM isolation (SmolVM/libkrun) | ❌                              | ❌        | ✅          | ❌             | Primary backend. Fork-based warm pool, 131 MB idle RSS per VM or resume in &lt; 200 ms. Docker actions run inside the VM. |
| Process execution (no container)   | ✅ (`-self-hosted`)             | ❌        | ✅          | —             |                                                                                                                           |
| macOS native runner                | ⚠️ (`-self-hosted` workaround) | ⚠️       | 🚧 planned | ✅             | preloop: Virtualization.framework-based ephemeral macOS VMs are designed but not yet shipped                              |
| Windows native runner              | ⚠️ (`-self-hosted` workaround) | ❌        | 🚧 planned | ✅             | preloop: QEMU-based ephemeral Windows VMs are designed but not yet shipped                                                |
| Custom platform images (`-P`)      | ✅                              | —        | N/A        | —             | act-specific concept                                                                                                      |
| Podman support                     | ✅                              | ❌        | ✅          | ✅             | act via `DOCKER_HOST`; preloop via runner backend                                                                         |
...
| Artifact v2 (Twirp + blob)              | ✅ (v3/v4) | ✅        | ✅       | ⚠️            |       |
| File-backed storage                     | ✅         | ✅        | ✅       | ✅             |       |
...
| NDJSON event stream (AI agent output) | ❌                   | ✅        | ✅       | —             | agent-ci: `--json` emits structured events. preloop: `events.ndjson` per run  |
| Pause-on-failure (interactive retry)  | ❌                   | ✅        | ✅       | —             | Both pause for in-place debugging; preloop adds step rewind, DAP, source sync |
| Commit status posting                 | ❌                   | ✅        | ✅       | —             | agent-ci: `--commit-status`. preloop: GitHub App integration                  |
| Workflow graph visualization          | ✅ (`act --graph`)   | ❌        | ❌       | —             |                                                                               |
| Event simulation                      | ✅ (`-e event.json`) | ⚠️       | ✅       | ⚠️            |                                                                               |
| Dry run / list                        | ✅ (`-l`, `-n`)      | ❌        | ❌       | ⚠️            |                                                                               |
...
4. `preloop-runner` (Rust reimplementation) acquires the job via broker polling
...
### preloop: Full AzDO wire protocol

preloop implements the complete Azure DevOps runner protocol:
...
- **Preloop (preloop)**: **31 / 39 (79.5%)** correct behavior (26/39 passed)
...
### preloop's known gaps (features it doesn't fully support)
...
|                     | act                                                            | preloop                                      |
| ------------------- | -------------------------------------------------------------- | -------------------------------------------- |
| **Stars**           | ~71,000                                                        | Newly open-sourced                           |
| **Language**        | Go                                                             | Rust                                         |
...
| **License**         | MIT                                                            | MIT; FSL-1.1-MIT for the control plane crate |


## 1.5. Also in the space: Gitea/Forgejo and GitHub-hosted

Two adjacent options are worth naming, because "run GitHub Actions locally"
often gets conflated with them:

- **Gitea/Forgejo Actions (`act_runner`)** — Gitea's runner is a maintained
fork of act ([gitea/act](https://gitea.com/gitea/act), merged into
`act_runner`) driven by the forge's own event system. You get Actions-like
workflows inside your self-hosted forge, with the same act-engine fidelity
caveats, and no GitHub protocol involvement (no check runs on GitHub, no
GitHub webhooks). Pick it when you already run a Gitea/Forgejo instance;
it is a forge feature, not a standalone CI engine.
- **GitHub-hosted runners** — the reference behavior and zero maintenance,
at the cost of hosted minutes and the dependency on GitHub availability.
Official **self-hosted runners + GitHub** are the protocol-compatible
baseline that preloop also speaks: preloop replaces the GitHub side of the wire
protocol, not the runner side.

The rest of this document is the four-way deep dive; Gitea Actions has its
own column since `act_runner` shares act's engine but diverges on several
rows (Connect RPC, per-repo tokens, patched artifact/cache support).
...
| Windows/macOS container workflows               | ❌ Linux Docker only   | 🚧 Linux via SmolVM today; macOS/Windows VM backends planned |
...
**preloop** reimplements the entire stack — both the server-side control plane and
the runner — in Rust. `preloop-runner` is a faithful port of `actions/runner`
...
The fundamental difference is scope: **act reimplements only the runner; preloop
