...
|---|---:|---|---|---|
| `L0/CommandLineParserL0.cs` | 5 | **FULL** | CLI command parsing and arg validation — all configure flags (replace, ephemeral, unattended, no-externals, work dir, runner-group), run/worker protocol path selection, remove token | `parse_configure`, `parse_run_defaults`, `parse_run_azdo`, `parse_remove`, `parse_worker`, `global_ca_bundle_arg`, `version_string_contains_protocol_compat`, `parse_configure_replace_flag`, `parse_configure_ephemeral_replace`, `parse_configure_no_externals`, `parse_configure_custom_work_dir`, `parse_configure_runner_group`, `parse_configure_defaults`, `parse_run_once_broker`, `parse_worker_azdo` |
...
| `L0/Listener/BrokerMessageListenerL0.cs` | 7 | **PARTIAL** | Broker message parsing (plaintext object/string bodies), session key extraction (unencrypted/missing/empty), HTTP error classification (401 unauthorized, 404/400 session expired), message dedup logic | `is_unauthorized_detects_401`, `is_unauthorized_rejects_other_status`, `is_unauthorized_rejects_non_http_error`, `is_session_expired_detects_404`, `is_session_expired_detects_400`, `is_session_expired_rejects_200`, `parse_message_body_plaintext_object`, `parse_message_body_plaintext_string`, `parse_message_body_empty_is_error`, `parse_message_body_no_body_field`, `extract_session_key_no_encryption_key`, `extract_session_key_empty_value`, `extract_session_key_unencrypted_returns_raw` |
| `L0/Listener/CommandSettingsL0.cs` | 30 | **PARTIAL** | CLI configure settings: all flags covered (replace, ephemeral, unattended, no-externals, work, runner-group, labels), run settings (once, via), defaults verification. Interactive prompt manager (7 tests) remains gap. | `parse_configure`, `parse_run_defaults`, `parse_run_azdo`, `parse_remove`, `parse_worker`, `parse_configure_replace_flag`, `parse_configure_ephemeral_replace`, `parse_configure_no_externals`, `parse_configure_custom_work_dir`, `parse_configure_runner_group`, `parse_configure_defaults`, `parse_run_once_broker`, `parse_worker_azdo` |
| `L0/Listener/Configuration/ArgumentValidatorTestsL0.cs` | 4 | **GAP** | Argument validator rules not directly tested | — |
| `L0/Listener/Configuration/ConfigurationManagerL0.cs` | 7 | **PARTIAL** | Settings round-trip, config lifecycle, ephemeral/hosted/v2 fields, auth migration credentials, BOM handling, camelCase JSON keys, default values | `round_trip_settings`, `config_lifecycle`, `rsa_params_field_names`, `strip_bom_works`, `runner_settings_ephemeral_fields_roundtrip`, `runner_settings_camel_case_json_keys`, `runner_settings_default_use_v2_flow_is_true`, `config_save_load_with_all_credential_fields` |
...
| `L0/Listener/Configuration/RunnerCredentialL0.cs` | 2 | **FULL** | RSA params field naming, credential data accessors (clientId, authorizationUrl), auth migration fields (enableAuthMigrationByDefault, authorizationUrlV2, oauthEndpointUrl) | `rsa_params_field_names`, `round_trip_settings`, `credential_data_accessors`, `credential_data_missing_fields`, `credential_data_auth_migration_fields` |
| `L0/Listener/ErrorThrottlerL0.cs` | 3 | **PARTIAL** | Error backoff logic verified in broker listener (consecutive_errors * 5 capped at 60s) and message listener; reset-after-success verified via consecutive_errors = 0 on Ok paths. Throttler not a separate unit — logic inline in broker/message listeners. | `is_unauthorized_detects_401`, `is_session_expired_detects_404` (error classification drives throttle/retry decisions) |
| `L0/Listener/JobDispatcherL0.cs` | 12 | **PARTIAL** | Worker message IPC serialization (job/cancel/shutdown), job spawn with request ID extraction (jobId + requestId fallback), worker dispatch + wait, cancel + grace period. Still missing: parallel job rejection, duplicate job ID handling (both are inline in broker_listener loop, not unit-testable without mocking HTTP). | `test_worker_dispatch_run_new_job`, `test_worker_dispatch_cancellation`, `worker_message_job_serialization`, `worker_message_cancel_serialization`, `worker_message_shutdown_serialization`, `worker_message_roundtrip_all_types`, `spawn_job_extracts_request_id_from_job_id`, `spawn_job_extracts_request_id_from_fallback` |
| `L0/Listener/MessageListenerL0.cs` | 9 | **PARTIAL** | Message type dispatch (PipelineAgentJobRequest, RunnerJobRequest, JobCancellation, AgentRefresh, unknown), session key extraction (unencrypted/missing/empty/encrypted-without-keypair) | `process_message_job_request`, `process_message_runner_job_request`, `process_message_cancellation_returns_none`, `process_message_agent_refresh_returns_none`, `process_message_unknown_type_returns_none`, `process_message_missing_type_returns_none`, `extract_session_key_optional_no_key`, `extract_session_key_optional_empty_value`, `extract_session_key_optional_unencrypted`, `extract_session_key_optional_encrypted_without_keypair` |
...
| `L0/ProcessExtensionL0.cs` | 1 | **NOT_APPLICABLE** | Official .NET ProcessExtension helper has no Rust equivalent; behavior covered through `process::invoke` tests. | — |
| `L0/ProcessInvokerL0.cs` | 12 | **FULL** | Process invocation covers stdout/stderr observed ordering, callback delivery, working directory, env propagation, non-zero exit codes, long output, SIGINT→SIGTERM cancellation, and cancellation errors. | `cancel_sends_sigint_before_hard_kill`, `cancel_falls_back_to_sigterm_when_sigint_is_ignored`, `invoke_captures_stdout_and_stderr_in_observed_order`, `invoke_streams_lines_to_callback_before_exit`, `invoke_sets_working_directory`, `invoke_propagates_environment`, `invoke_returns_nonzero_exit_code`, `invoke_handles_long_output_without_loss`, `cancelled_process_returns_error` |
| `L0/RunnerWebProxyL0.cs` | 18 | **NOT_APPLICABLE** | aksh-runner delegates HTTP proxy discovery/bypass behavior to reqwest/system proxy support; Docker/container proxy injection is covered under ContainerOperationProvider. | — |
| `L0/Sdk/ExpressionParserL0.cs` | 4 | **OUTSIDE_RUNNER** | Expression parser equivalent is aksh-gha-expressions, not aksh-runner | — |
| `L0/Sdk/LaunchWebApi/LaunchHttpClientL0.cs` | 2 | **NOT_APPLICABLE** | Launch client is not used by the broker-only aksh runner path; no separate launch service client exists. | — |
...
| `L0/Sdk/RSWebApi/RunServiceHttpClientL0.cs` | 1 | **FULL** | Run/results service HTTP behavior: empty success body maps to JSON null, client error bodies preserved as typed HttpError, long-poll 202/204 maps to no message, PUT signed blob error bodies preserved, run-service base URL trims trailing slash, results Twirp endpoints use stable paths. | `post_json_with_auth_handles_empty_success_response`, `post_json_with_auth_preserves_client_error_body`, `post_json_with_auth_headers_handles_empty_success_response`, `get_long_poll_treats_accepted_as_no_message`, `get_long_poll_treats_no_content_as_no_message`, `get_long_poll_preserves_error_body`, `put_bytes_preserves_client_error_body`, `new_trims_trailing_slashes_from_base_url`, `new_preserves_base_url_without_trailing_slash`, `update_workflow_steps_endpoint_path_matches_twirp_shape`, `signed_blob_endpoint_paths_match_receiver_service` |
| `L0/Sdk/WellKnownRegularExpressionsL0.cs` | 9 | **NOT_APPLICABLE** | Official well-known regex constants are .NET SDK internals; aksh implements concrete parsing where needed in commands/matchers instead of exposing this constant surface. | — |
...
| `L0/Util/ArgUtilL0.cs` | 7 | **NOT_APPLICABLE** | Official .NET argument utility helpers are not mirrored as a public Rust surface; clap and domain parsers cover executable behavior. | — |
| `L0/Util/IOUtilL0.cs` | 22 | **NOT_APPLICABLE** | Official .NET IO helper utility is not mirrored; file-command, log upload, and workspace behavior are tested through runner-visible paths. | — |
| `L0/Util/StringUtilL0.cs` | 6 | **NOT_APPLICABLE** | Official .NET string helper utility is not mirrored; string parsing behavior is tested at workflow command/template/matcher boundaries. | — |
| `L0/Util/TaskResultUtilL0.cs` | 2 | **NOT_APPLICABLE** | Official task-result merge helper is not mirrored; job/step result propagation is covered by StepsRunner/JobRunner tests. | — |
| `L0/Util/UrlUtilL0.cs` | 5 | **NOT_APPLICABLE** | Official URL utility helper is not mirrored; concrete URL construction is tested in broker/run/results clients. | — |
| `L0/Util/VssUtilL0.cs` | 1 | **NOT_APPLICABLE** | Official VSS utility helper is not mirrored in the broker-only Rust runner. | — |
| `L0/Util/WhichUtilL0.cs` | 7 | **NOT_APPLICABLE** | Official which/path-search utility is not mirrored as a public surface; action/script execution resolves concrete runtimes directly. | — |
...
| `L0/Worker/PipelineDirectoryManagerL0.cs` | 8 | **NOT_APPLICABLE** | Official pipeline directory tracking manager is not mirrored; aksh uses job fileTable/workDirectory and container mount paths directly. | — |
...
| `L0/Worker/TrackingManagerL0.cs` | 4 | **NOT_APPLICABLE** | Official workspace tracking config persistence is not mirrored in aksh local runner state. | — |
...
| `L0/Worker/WorkerL0.cs` | 2 | **FULL** | Worker top-level run loop, cancellation dispatch, IPC message serialization, request ID extraction | `test_worker_dispatch_run_new_job`, `test_worker_dispatch_cancellation`, `worker_message_job_serialization`, `worker_message_cancel_serialization`, `worker_message_shutdown_serialization`, `spawn_job_extracts_request_id_from_job_id`, `spawn_job_extracts_request_id_from_fallback` |
