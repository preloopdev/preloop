#!/usr/bin/env bash
# Capture concurrency scenarios on local aksh (server + preloop-runner) in
...
try:
    ...
    if a and a.get("run_id"):
        ...
        (d/"summary.json").write_text(json.dumps({
            ...
            "conclusion": "failure", "status": "completed", "jobs": [], "source": "preloop",
    ...
    (d/"summary.json").write_text(json.dumps({
        ...
        "source": "preloop", "http_status": e.code, "body": body,
    }, indent=2))
