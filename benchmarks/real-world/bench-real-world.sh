#!/usr/bin/env bash
# bench-real-world.sh — Benchmark real-world repos with preloop/official runner
# Overnight set: preloop, agent-ci, act, redis-py, docker-compose.
# Usage: ./bench-real-world.sh <preloop|agent-ci|act|redis-py|docker-compose> [preloop|official|both] [runs]
set -euo pipefail

REPO="${1:?Usage: $0 <preloop|agent-ci|act|redis-py|docker-compose> [preloop|official|both] [runs]}"
MODE="${2:-preloop}"
RUNS="${3:-1}"
...
configure_preloop() {
  ...
  local t0=$(ms)
  "$AKSH/preloop-runner" --runner-root "$root" configure \
...
submit_workflow() {
  local t0=$(ms)
  local out
  out=$("$AKSH/preloop-runner-client" --server "http://127.0.0.1:$PORT" \
...
run_preloop_runner() {
  local root="$1" log="$2"
  local t0=$(ms)
  RUST_LOG=info "$AKSH/preloop-runner" --runner-root "$root" run --once > "$log" 2>&1 || true
  local t1=$(ms)
  echo $((t1-t0))
}
...
extract_step_timings() {
  ...
  if [ "$runner" = "preloop" ]; then
    # Parse preloop runner logs: timestamps + step names
...
do_preloop_run() {
  local n=$1
  echo ""
  echo "--- preloop run $n/$RUNS ---"
  start_fresh_server

  local root="/tmp/bench-preloop-$n"
  configure_preloop "$root" "bench-preloop-$n"
  submit_workflow

  local log="/tmp/bench-preloop-$REPO-$n.log"
  echo "  running..."
  local run_ms=$(run_preloop_runner "$root" "$log")
  ...
  # Step timings
  echo "  steps:"
  extract_step_timings "$log" "preloop"

  echo "{\"runner\":\"preloop\",\"repo\":\"$REPO\",\"n\":$n,\"runner_ms\":$run_ms,\"result\":\"$result\"}" \
...
for i in $(seq 1 "$RUNS"); do
  case "$MODE" in
    preloop)     do_preloop_run "$i" ;;
    official) do_official_run "$i" ;;
    both)     do_preloop_run "$i"; do_official_run "$i" ;;
  esac
done
...
echo ""
echo "Logs: /tmp/bench-{preloop,official}-${REPO}-*.log"
