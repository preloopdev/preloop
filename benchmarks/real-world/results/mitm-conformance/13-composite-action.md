...
- official capture: `/Users/bnjoroge/cachingv4/.runner-watch/golden/v2.335.1/13-composite-action`
- aksh capture: `/Users/bnjoroge/cachingv4/experiments/mitm/captures/aksh/13-composite-action/latest`
- official summary: status=ok flows=35
- aksh summary: status=None flows=41
...
| `POST results-receiver.actions.githubusercontent.com/twirp/results.services.receiver.Receiver/GetStepLogsSignedBlobURL` | 4 | 3 ⚠ |
...
@@ -2,12 +2,12 @@
   "authorization": {
     "publicKey": {
       "exponent": "AQAB",
-      "modulus": "{token}+{token}+{token}/{token}+{token}+RU0lwrMzLkOcX6iz34b/{token}/{token}=="
+      "modulus": "{token}+t4RjM7vBD8g+bZAsrjuWE4caRFuXrXh+dNlrkA+{token}/C3kBf6Xx3KzzDdBVHRD+UXAL3bDA9/{token}/{token}=="
     }
   },
   "createdOn": "{time}",
-  "disableUpdate": false,
