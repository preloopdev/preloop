#!/usr/bin/env bash
# capture-preloop-mitm.sh — Capture preloop-runner MITM flows against GitHub
# Usage: ./capture-preloop-mitm.sh <scenario> [vm=bench-aksh-4]
...
smolvm machine exec --name "$VM" -- bash -c "
  ...
  killall preloop-runner 2>/dev/null || true
  sleep 1
" 2>/dev/null || true
