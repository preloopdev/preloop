#!/usr/bin/env bash
# 10-repo conformance campaign — fresh repos NOT in previous campaigns.
# Previous (41, do not repeat): cli, pydantic, serde, valkey, deno, grafana,
#   express, tokio, runc, flask, gin, json-c, nlohmann/json, node-fetch, bat,
#   vite, uv, nextcloud, caddy, bento, agent-ci, openclaw, buzz, qm, runner,
#   ripgrep, click, yq, axios, fd, zoxide, fzf, golangci-lint, jq, starship,
#   cobra, typescript, nodejs, react, vscode, spark.
# New picks (diverse, popular, ubuntu-dominated, ARM64-safe; amd64-only
# container jobs are a known gap so none are selected):
#   - spf13/hugo (Go app, test.yml)
#   - grpc/grpc-go (Go lib, testing.yml)
#   - pytest-dev/pytest (Python, test.yml)
#   - django/django (Python + DB matrix, tests.yml)
#   - laravel/framework (PHP + service containers mysql/redis/memcached, tests.yml)
#   - fmtlib/fmt (C++ cmake, linux.yml)
#   - nushell/nushell (Rust binary, ci.yml)
#   - jekyll/jekyll (Ruby, ci.yml)
#   - junit-team/junit5 (Java, ci.yml; uses runs-on: ubuntu-slim -> extra label)
#   - testcontainers/testcontainers-go (Go + docker workloads, ci.yml)
#
# Isolation: PRELOOP_RUNNER_POOL_SIZE=0 — the supervisor keeps one packed
# golden and forks a DISPOSABLE smolvm VM per job. No VM is reused across
# jobs; nothing executes on the host.
set -Eeuo pipefail
ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
WORKSPACE_ROOT="${CONFORMANCE_WORKSPACE_ROOT:-/tmp/preloop-conformance-10repos/workspaces}"
OUTPUT_ROOT="${CONFORMANCE_OUTPUT_ROOT:-$ROOT/benchmarks/real-world/results/conformance-10repos}"
CAMPAIGN_HOME="${CONFORMANCE_HOME:-/tmp/preloop-10repos-home}"
PORT="${CONFORMANCE_PORT:-9199}"
POLL_SECONDS="${CONFORMANCE_POLL_SECONDS:-15}"
...
usage() {
  cat <<'EOF'
Usage: conformance-10repos.sh [all|<repo>] [--workflow NAME]
Repos: hugo grpc pytest django laravel fmt nushell jekyll junit testcontainers
EOF
}
fail() { echo "conformance-10repos: $*" >&2; exit 1; }
target_cfg() {
  case "$1" in
    hugo/test)
      echo "hugo https://github.com/spf13/hugo.git master .github/workflows/test.yml push refs/heads/master" ;;
    grpc/test)
      echo "grpc https://github.com/grpc/grpc-go.git master .github/workflows/testing.yml push refs/heads/master" ;;
    pytest/test)
      echo "pytest https://github.com/pytest-dev/pytest.git main .github/workflows/test.yml push refs/heads/main" ;;
    django/tests)
      echo "django https://github.com/django/django.git main .github/workflows/tests.yml push refs/heads/main" ;;
    laravel/tests)
      echo "laravel https://github.com/laravel/framework.git 13.x .github/workflows/tests.yml push refs/heads/13.x" ;;
    fmt/linux)
      echo "fmt https://github.com/fmtlib/fmt.git main .github/workflows/linux.yml push refs/heads/main" ;;
    nushell/ci)
      echo "nushell https://github.com/nushell/nushell.git main .github/workflows/ci.yml push refs/heads/main" ;;
    jekyll/ci)
      echo "jekyll https://github.com/jekyll/jekyll.git master .github/workflows/ci.yml push refs/heads/master" ;;
    junit/ci)
      echo "junit https://github.com/junit-team/junit5.git main .github/workflows/ci.yml push refs/heads/main" ;;
    testcontainers/ci)
      echo "testcontainers https://github.com/testcontainers/testcontainers-go.git main .github/workflows/ci.yml push refs/heads/main" ;;
    *) return 1 ;;
  esac
}
all_targets() {
  echo "hugo/test grpc/test pytest/test django/tests laravel/tests fmt/linux nushell/ci jekyll/ci junit/ci testcontainers/ci"
}
repo_targets() {
  case "$1" in
    hugo) echo "hugo/test" ;;
    grpc) echo "grpc/test" ;;
    pytest) echo "pytest/test" ;;
    django) echo "django/tests" ;;
    laravel) echo "laravel/tests" ;;
    fmt) echo "fmt/linux" ;;
    nushell) echo "nushell/ci" ;;
    jekyll) echo "jekyll/ci" ;;
    junit) echo "junit/ci" ;;
    testcontainers) echo "testcontainers/ci" ;;
    *) return 1 ;;
  esac
}
...
prepare_golden_home() {
  ...
  # Ask the server code itself for the exact payload path (stem + current
  # environment fingerprint). A hand-rolled hash drifts whenever the bake,
  # toolchains, or node pins change, and a miss means a full 9GB rebuild.
...
start_server() {
  ...
  rm -f "$log"
  # smolvm/libkrun must be a matched pair: 1.14.6 demands krun_set_cpu_template,
  # which neither ~/.smolvm/lib (1.8.2-era) nor Homebrew upstream libkrun
  # provides, so every boot dies in load libkrun. 1.8.2 worked but its fork
  # path is broken (clone boots die code 133; every job is a ~30 GB full
  # materialization). 1.15.0 + its bundled libs forks correctly (second job
  # boots in ~60s, clones ~1 GB). Shadow PATH so the server's orchestrator
  # forks the working binary.
  ...
  PRELOOP_RUNNER_NAME_PREFIX="conformance-10repos" \
  ...
  PRELOOP_RUNNER_STORAGE_GB="${PRELOOP_RUNNER_STORAGE_GB:-100}" \
  PRELOOP_RUNNER_LABELS="${PRELOOP_RUNNER_LABELS:-X64,ubuntu-arm64-small,ubuntu-x64-small,ubuntu-x64,ubuntu-latest,ubuntu-22.04,ubuntu-24.04,ubuntu-20.04,ubuntu-slim}" \
  ...
  for _ in $(seq 1 120); do
    if curl -fsS --max-time 5 "http://127.0.0.1:$PORT/healthz" >/dev/null 2>&1; then
      echo "=== preloop server ready on :$PORT using $OFFICIAL_GOLDEN_ARTIFACT (pool=$POOL_SIZE) ==="
      return 0
    fi
