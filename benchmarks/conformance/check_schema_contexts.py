...
AUTHORITATIVE_REPO = "actions/languageservices"
AUTHORITATIVE_PATH = "workflow-parser/src/workflow-v1.0.json"
...
# Each entry ties the schema contract to the parser field that invokes it.
# The checker searches forward from the field anchor and fails if the expected
# validator/helper is not wired nearby.
CONTEXT_VALIDATION_SITES: dict[str, tuple[tuple[str, str], ...]] = {
    "run-name": (("if let Some(run_name)", "CTX_RUN_NAME"),),
    "workflow-env": (("validate_env_expressions(&workflow.env", "CTX_WORKFLOW_ENV"),),
    "workflow-concurrency": (
        ("if let Some(concurrency) = &workflow.concurrency", "CTX_WORKFLOW_CONCURRENCY"),
    ),
    "job-if": (("if let Some(cond) = &job.if_condition", "CTX_JOB_IF"),),
    "strategy": (("&job.strategy.matrix", "CTX_STRATEGY"),),
    "runs-on": (("match &job.runs_on", "CTX_JOB_RUNS_ON"),),
    "job-env": (("validate_env_expressions(&job.env", "CTX_JOB_ENV"),),
    "job-concurrency": (
        (
    ...
    "job-defaults-run": (("if let Some(defaults) = &job.defaults", "CTX_JOB_DEFAULTS_RUN"),),
    "container": (
        ("if let Some(container) = &job.container", "validate_container_expressions(container)"),
        ("fn validate_container_expressions", "CTX_JOB_CONTAINER"),
    ),
    "services": (
        ("if let Some(services) = &job.services", "validate_container_expressions(service)"),
        ("fn validate_container_expressions", "CTX_JOB_CONTAINER"),
    ),
    "services-container": (
        ("if let Some(services) = &job.services", "validate_container_expressions(service)"),
    ...
    "boolean-strategy-context": (
        ("JobContinueOnError::Expression(expr)", "CTX_JOB_RUNS_ON"),
    ),
    "string-strategy-context": (("if let Some(name) = &job.name", "CTX_JOB_RUNS_ON"),),
    "string-runner-context": (("for (output_name, output)", "CTX_RUNNER"),),
    "step-continue-on-error": (
        ("&step.continue_on_error", "CTX_STEP_CONTINUE_ON_ERROR"),
    ),
    "step-if": (("if let Some(cond) = &step.if_condition", "CTX_STEP_IF"),),
    "step-env": (("validate_env_expressions(&step.env", "CTX_STEP_ENV"),),
    "step-name": (("if let Some(name) = &step.name", "CTX_STEP_NAME"),),
    "step-timeout-minutes": (("&step.timeout_minutes", "CTX_STEP_TIMEOUT"),),
    "step-with": (("for val in step.with.values()", "CTX_STEP_WITH"),),
    "string-steps-context": (
        ("if let Some(run) = &step.run", "CTX_STEP_RUN"),
        ("if let Some(wd) = &step.working_directory", "CTX_STEP_WORKING_DIR"),
    ),
}

# Context-less schema fields inherit an empty context set. Keep these path
# bindings explicit because the same named scalar (for example `shell`) can
# inherit a non-empty context under job defaults and an empty one elsewhere.
PATH_CONTEXT_BINDINGS: dict[str, tuple[str, ...]] = {
    "defaults.run.shell": ("CTX_NO_EXPRESSIONS",),
    "defaults.run.working-directory": ("CTX_NO_EXPRESSIONS",),
    "jobs.<*>.defaults.run.shell": ("CTX_JOB_DEFAULTS_RUN",),
    "jobs.<*>.defaults.run.working-directory": ("CTX_JOB_DEFAULTS_RUN",),
    "jobs.<*>.steps[*].shell": ("CTX_NO_EXPRESSIONS",),
}

PATH_VALIDATION_SITES: dict[str, tuple[tuple[str, str], ...]] = {
    "defaults.run.shell": (
        ("if let Some(defaults) = &workflow.defaults", "CTX_NO_EXPRESSIONS"),
    ),
    "defaults.run.working-directory": (
        ("if let Some(defaults) = &workflow.defaults", "CTX_NO_EXPRESSIONS"),
    ),
    "jobs.<*>.defaults.run.shell": (
        ("if let Some(defaults) = &job.defaults", "CTX_JOB_DEFAULTS_RUN"),
    ),
    "jobs.<*>.defaults.run.working-directory": (
        ("if let Some(defaults) = &job.defaults", "CTX_JOB_DEFAULTS_RUN"),
    ),
    "jobs.<*>.steps[*].shell": (("if let Some(shell) = &step.shell", "CTX_NO_EXPRESSIONS"),),
}

# This is an intentional temporary narrowing rather than an advertised
# capability: the parser materializes StepPlan before any prior step exists,
# so it cannot resolve `steps.*` in timeout-minutes yet.
CONTEXT_DIFF_ALLOWLIST: dict[str, dict[str, Any]] = {
    "step-timeout-minutes": {
        "missing": ("steps",),
        "extra": (),
        "reason": (
            "Step timeout expressions are currently resolved during plan expansion; "
            "runner-side deferred timeout tokens are not implemented."
        ),
    },
}
...
def load_json(path: Path, label: str) -> dict[str, Any]:
    """Load a JSON object or fail the audit with source context."""
    try:
...
def verify_schema_source(
    ...
    """Verify immutable provenance for the vendored workflow schema."""
    required = {"repo", "path", "commit", "raw_url", "sha256"}
    missing = sorted(required - source.keys())
    if missing:
        raise AuditError(f"schema source metadata missing keys: {', '.join(missing)}")
    repo, source_path = source["repo"], source["path"]
    if not allow_source_override and (repo, source_path) != (
        AUTHORITATIVE_REPO,
        AUTHORITATIVE_PATH,
    ):
        raise AuditError(
            "schema source must be "
            f"{AUTHORITATIVE_REPO}/{AUTHORITATIVE_PATH}; "
            "pass --allow-source-override for an intentional alternate source"
        )
    if not isinstance(repo, str) or not isinstance(source_path, str):
        raise AuditError("schema source repository and path must be strings")
    commit = source["commit"]
    if not isinstance(commit, str) or not re.fullmatch(r"[0-9a-f]{40}", commit):
        raise AuditError("schema source commit must be a 40-character lowercase SHA")
    raw_url = source["raw_url"]
    expected_url = f"https://raw.githubusercontent.com/{repo}/{commit}/{source_path}"
    if raw_url != expected_url:
        raise AuditError(f"schema source raw_url must be exactly {expected_url}")
    recorded_hash = source["sha256"]
    if not isinstance(recorded_hash, str) or not re.fullmatch(r"[0-9a-f]{64}", recorded_hash):
        raise AuditError("schema source sha256 must be 64 lowercase hexadecimal characters")
    actual_hash = hashlib.sha256(schema_path.read_bytes()).hexdigest()
    if actual_hash != recorded_hash:
        raise AuditError(
            f"vendored schema sha256 mismatch: metadata={recorded_hash} actual={actual_hash}"
        )
...
def parse_context_constants(source: str) -> dict[str, dict[str, Any]]:
    """Parse and resolve every CTX_* Rust constant declaration."""
    ...
    def resolve(name: str) -> list[str]:
        """Resolve a CTX_* alias chain to its literal values."""
...
def named_spec(spec: Any) -> str | None:
    """Return the named type referenced by a schema specification."""
...
def schema_kinds(definitions: dict[str, Any], spec: Any, stack: tuple[str, ...] = ()) -> set[str]:
    """Resolve the primitive kinds accepted by one schema specification."""
...
def walk_schema(
    ...
    """Return explicit context definitions and effective contexts by path."""

    context_paths: dict[str, set[str]] = {}
    path_contexts: dict[str, set[tuple[str, ...]]] = {}

    def walk(spec: Any, path: str, inherited_context: tuple[str, ...], stack: tuple[str, ...]) -> None:
        """Walk one reachable schema node while carrying inherited contexts."""
        name = named_spec(spec)
        if name is None:
        ...
        path_contexts.setdefault(path or "<root>", set()).add(context)
        if "context" in node:
    ...
    walk("workflow-root", "", (), ())
    return context_paths, path_contexts


def normalize_context(value: str) -> str:
    """Canonicalize a context or function contract for comparison."""
    return re.sub(r"\s+", "", value).lower()


def context_diff(schema_values: list[str], ours: list[str]) -> tuple[list[str], list[str]]:
    """Return schema contracts missing from or added by the parser."""
    schema_by_contract = {normalize_context(value): value for value in schema_values}
    ours_by_contract = {normalize_context(value): value for value in ours}
    missing = [
        schema_by_contract[key]
        for key in sorted(schema_by_contract.keys() - ours_by_contract.keys())
    ]
    extra = [
        ours_by_contract[key]
        for key in sorted(ours_by_contract.keys() - schema_by_contract.keys())
    ]
    return missing, extra


def verify_validation_sites(
    source: str,
    sites: dict[str, tuple[tuple[str, str], ...]],
) -> list[str]:
    """Verify each field anchor invokes its declared validator nearby."""
    errors: list[str] = []
    for contract, checks in sites.items():
        for anchor, expected in checks:
            start = source.find(anchor)
            if start < 0:
                errors.append(f"{contract}: validation anchor missing: {anchor}")
                continue
            if expected not in source[start : start + 1200]:
                errors.append(
                    f"{contract}: `{expected}` is not wired near `{anchor}`"
                )
    return errors


def struct_body(source: str, name: str) -> tuple[str, int] | None:
    """Return a Rust struct body and its first source line."""
...
def enum_body(source: str, name: str) -> tuple[str, int] | None:
    """Return a Rust enum body and its first source line."""
...
def rust_target_type(source: str, target: str) -> tuple[str | None, int | None]:
    """Locate the declared Rust type for a configured model target."""
    if "." not in target:
...
def rust_kinds(type_name: str | None, target: str) -> set[str]:
    """Map a Rust model type to the schema kinds it accepts."""
...
def schema_type_text(kinds: set[str]) -> str:
    """Render a stable schema-kind set for audit output."""
    return "{" + ", ".join(sorted(kinds)) + "}"


def type_compatible(model: set[str], schema: set[str]) -> bool:
    """Require equivalent accepted kinds, including one-of models."""
    if "one-of" in model:
        return model == {"one-of"} and len(schema) > 1
...
def parse_args() -> argparse.Namespace:
    """Parse audit paths and the explicit alternate-source opt-in."""
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="permit metadata for an intentional alternate schema repository/path",
    )
...
def main() -> int:
    """Run the provenance, context, call-site, and model audit."""
    args = parse_args()
    try:
        ...
        context_paths, path_contexts = walk_schema(definitions)
    ...
    print("\nContext fields:")
    missing_sites = sorted(set(CONTEXT_BINDINGS) - set(CONTEXT_VALIDATION_SITES))
    extra_sites = sorted(set(CONTEXT_VALIDATION_SITES) - set(CONTEXT_BINDINGS))
    if missing_sites or extra_sites:
        errors.append(
            f"context validation-site table mismatch: missing={missing_sites} extra={extra_sites}"
        )
    site_errors = verify_validation_sites(eval_source, CONTEXT_VALIDATION_SITES)
    errors.extend(site_errors)
    for error in site_errors:
        print(f"DIVERGENCE callsite {error}")
    ...
    for definition in sorted(context_paths):
        ...
        if definition in CONTEXT_BINDINGS:
            ...
            allowlisted_diffs: list[tuple[str, list[str], list[str], str]] = []
            for constant in binding:
                ...
                if not missing and not extra:
                    continue
                allowance = CONTEXT_DIFF_ALLOWLIST.get(definition)
                if allowance and {
                    normalize_context(value) for value in missing
                } == {
                    normalize_context(value) for value in allowance["missing"]
                } and {
                    normalize_context(value) for value in extra
                } == {
                    normalize_context(value) for value in allowance["extra"]
                }:
                    allowlisted_diffs.append(
                        (constant, missing, extra, allowance["reason"])
                    )
                else:
            ...
            elif allowlisted_diffs:
                for constant, missing, extra, reason in allowlisted_diffs:
                    print(
                        f"ALLOWLISTED {definition} paths={paths} "
                        f"{constant}: missing={missing or '-'} extra={extra or '-'} "
                        f"class=wrongly-rejects source={location} reason={reason}"
                    )
            else:
                print(
    ...
    print("\nInherited/empty context fields:")
    path_site_errors = verify_validation_sites(eval_source, PATH_VALIDATION_SITES)
    errors.extend(path_site_errors)
    for error in path_site_errors:
        print(f"DIVERGENCE callsite {error}")
    for path, binding in sorted(PATH_CONTEXT_BINDINGS.items()):
        bound_constants.update(binding)
        effective_contexts = path_contexts.get(path)
        if not effective_contexts:
            errors.append(f"{path}: schema path not found")
            print(f"DIVERGENCE {path}: schema path not found")
            continue
        missing_constants = [name for name in binding if name not in constants]
        if missing_constants:
            errors.append(
                f"{path}: binding references missing {', '.join(missing_constants)}"
            )
            print(f"DIVERGENCE {path}: missing constants={missing_constants}")
            continue
        path_mismatches: list[str] = []
        for constant in binding:
            for effective in effective_contexts:
                missing, extra = context_diff(
                    list(effective), constants[constant]["values"]
                )
                if missing or extra:
        ...
        if path_mismatches:
            detail = "; ".join(path_mismatches)
            errors.append(f"{path}: {detail}")
            print(f"DIVERGENCE {path}: {detail}")
        else:
            print(
                f"OK {path} contexts={sorted(effective_contexts)} "
                f"constant={', '.join(binding)}"
            )
