#!/usr/bin/env bash
# batch-preloop-compare.sh — Run all scenarios against preloop server in a persistent VM
...
for scenario in $SCENARIOS; do
    log "═══ $scenario ═══"
    RESULT_DIR="$RESULTS_BASE/$scenario/preloop-server"
    rm -rf "$RESULT_DIR"
...
" <<< "$MODIFIED_YAML" > "$PAYLOAD_FILE"
    ...
    preloop_status=$(python3 -c "import json; print(json.load(open('$RESULT_DIR/status.json'))['status'])" 2>/dev/null || echo unknown)
    ...
    echo "{\"server\":\"preloop\",\"conclusion\":\"$preloop_status\"}" > "$RESULT_DIR/summary.json"
    ...
    if [ "$gh_c" = "missing" ] || [ "$gh_c" = "cancelled" ]; then
        log "preloop=$preloop_status (no valid GitHub baseline)"
        echo "$job_details"
    elif [ "$gh_c" = "$preloop_status" ]; then
        green "  ✅ $scenario: MATCH (both=$preloop_status)"
        echo "$job_details"
    else
        red "  ❌ $scenario: MISMATCH (github=$gh_c preloop=$preloop_status)"
        echo "$job_details"
    fi
    echo ""
done
