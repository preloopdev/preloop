...
SERVER_PORT = 9191
RESULTS_FILE = "benchmarks/compatibility/runner/behavior/conformance-preloop-new.jsonl"
...
for name, event in scenarios:
    ...
    state_dir = Path(tempfile.mkdtemp(prefix=f"preloop-{name[:10]}-"))
    ...
    try:
        ...
        c = subprocess.run(
            ["target/release/preloop-runner", "--runner-root", str(runner_root),
             ...
             "--token", "dummy-token", "--name", f"preloop-{name[:20]}",
        ...
        s = subprocess.run(
            ["target/debug/preloop-runner-client", "--server",
             f"http://127.0.0.1:{SERVER_PORT}",
             "submit", "-W", str(wf_path),
             "--event", event, "--repository", "preloopdev/preloop",
