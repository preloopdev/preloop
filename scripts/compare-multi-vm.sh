...
smolvm machine exec --name "$SERVER" -- bash -lc "
  ...
  cp /tmp/server.log /workspace/benchmarks/compatibility/server/behavior/$SCENARIO/preloop-multi/server-start.log
" >/dev/null
...
for i in $(seq 1 "$RUNNERS"); do
  ...
  smolvm machine exec --name "$vm" -- bash -lc "
    ...
    cp /tmp/config.log /workspace/benchmarks/compatibility/server/behavior/$SCENARIO/preloop-multi/runner-$i/config.log
    cp -a /tmp/runner/_diag/. /workspace/benchmarks/compatibility/server/behavior/$SCENARIO/preloop-multi/runner-$i/diag/ 2>/dev/null || true
    exit \$rc
